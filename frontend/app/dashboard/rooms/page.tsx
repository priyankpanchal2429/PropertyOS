  ...
  return (
    <div className="max-w-[1400px] mx-auto px-4 py-6 md:py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 items-start">
        ...
        {/* LEFT PANEL: Elevator / Building Panel */}
        <div className="lg:sticky lg:top-24 space-y-4">
          <div className="rounded-2xl p-5 border bg-[var(--card)] border-[var(--border)] shadow-sm flex flex-col items-center">
            <div className="w-full flex items-center gap-2 mb-6 pb-2 border-b border-[var(--border)]">
              <Building2 className="h-4 w-4 text-cyan-500" />
              <span className="text-xs font-black uppercase tracking-wider text-muted-foreground">Elevator Deck</span>
            </div>

            {/* Elevator Interface Console */}
            <div className="bg-muted/10 border border-[var(--border)] p-4 rounded-3xl w-24 flex flex-col items-center gap-4 relative">
              <div className="w-2.5 h-2.5 rounded-full bg-cyan-500/80 animate-ping absolute -top-1 right-8" />
              <div className="w-1.5 h-1.5 rounded-full bg-cyan-500 absolute -top-0.5 right-8.5" />
              
              {/* Floor 2 Button */}
              <button
                onClick={() => toggleFloor(2)}
                className={`w-12 h-12 rounded-full border-2 flex flex-col items-center justify-center cursor-pointer transition-all duration-300 font-black text-sm relative ${
                  !collapsedFloors[2] 
                    ? 'border-cyan-500 bg-cyan-500/10 text-cyan-500' 
                    : 'border-[var(--border)] bg-transparent text-muted-foreground hover:border-foreground/50'
                }`}
              >
                2F
                <span className="text-[8px] font-bold absolute -bottom-1 bg-background px-1 rounded border">
                  {floorCounts[2]}
                </span>
              </button>
              ...
              <div className="text-[8px] font-black uppercase text-muted-foreground tracking-widest mt-2 border-t pt-2 w-full text-center">
                PANEL
              </div>
            </div>
          </div>
